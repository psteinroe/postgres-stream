...
const PIPELINE_ID_LABEL: &str = "stream_id";

/// Label key for application type.
const APP_TYPE_LABEL: &str = "app_type";

/// Application type value for the binary.
const APP_TYPE_VALUE: &str = "postgres-async-triggers-app";
...
///
/// This function should be called after [`init_metrics`]
/// to ensure the metrics recorder is installed.
pub fn spawn_jemalloc_metrics_task(stream_id: u64) {
    register_jemalloc_metrics();
    log_jemalloc_config();
    ...
    tokio::spawn(async move {
        ...
        loop {
            ...
            // Update gauges with stream_id and app_type labels.
            gauge!(
                JEMALLOC_ALLOCATED_BYTES,
                PIPELINE_ID_LABEL => stream_id_str.clone(),
